...
def scout_bee_phase():
    if abandoned:
        for bee in abandoned:
            solution_set[bee] = produce_solution()
            stagnation[bee] = 0
            abandoned.pop()
...
def swap(type, bee): #types: (1)Employed (2)Onlooker
    ...
    if random.random() < 0.7:
        ...
        if get_swappable_slots([course, day, period, room], solution):
            available_slots = get_swappable_slots([course, day, period, room], solution)
            slot_index = 0
            least_difference = float("inf")
            i = 0 
            for slot in available_slots:
